...
Para evaluar la robustez del sistema, se diseñaron y simularon dos entornos distintos en Webots: un entorno simple con pocos obstáculos aislados (5 cilindros rojos) que permite trayectorias abiertas, y un entorno complejo con alta densidad de obstáculos (10 cilindros) y una configuración de pasillos estrechos delimitados por muros grises longitudinales. A partir de la ejecución del controlador en ambos escenarios, se analizó el comportamiento del robot bajo los siguientes criterios:

* **Estabilidad del movimiento:** En zonas abiertas y frente a aproximaciones directas a los cilindros, el desplazamiento del robot fue estable y lineal. La aplicación del filtro de Kalman fue determinante en este aspecto, ya que al reducir la varianza de la señal de los sensores, se evitó que el controlador diferencial de los motores oscilara de forma errática debido al ruido de medición.
* **Cantidad de giros innecesarios:** La implementación del estimador de estado eliminó los falsos positivos generados por las fluctuaciones crudas del infrarrojo. Esto garantizó que el robot solo activara la rutina de evasión cuando el obstáculo cruzaba realmente el umbral crítico, evitando comportamientos de titubeo o giros al vacío en los espacios libres del entorno simple.
* **Capacidad para evitar colisiones y limitaciones de hardware simulado:** En el primer escenario, el robot evadió exitosamente los obstáculos rojos, cuya superficie curva dispersa eficientemente el haz infrarrojo hacia el emisor. No obstante, en el entorno complejo, el sistema presentó fallos críticos frente a los muros grises lisos. Como se evidenció en la simulación, el robot chocó tangencialmente y quedó atascado al ingresar a un pasillo estrecho. Esta colisión múltiple ocurrió por un factor técnico: un fenómeno de reflexión especular, donde los muros lisos desviaron el haz infrarrojo al entrar en diagonal, retrasando la detección hasta el punto de impacto y encajonando físicamente al chasis.
* **Diferencias entre mediciones crudas, filtradas y fusionadas:** El análisis de los gráficos generados demuestra que las mediciones crudas presentan una varianza que hace inviable el control reactivo directo. La señal procesada mediante media móvil logró suavizar el ruido, pero introdujo un retardo de fase (delay) considerable, lo cual es crítico en aproximaciones a alta velocidad. Finalmente, el filtro de Kalman, implementado como un estimador de estado de velocidad cero, demostró el desempeño más robusto: logró reducir la covarianza del error de medición manteniendo un mejor seguimiento de la dinámica de la señal original, superando las limitaciones del filtro promediador simple.
